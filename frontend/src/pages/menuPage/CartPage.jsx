...
const CartPage = () => {
  ...
  const handleIncrease = async (item) => {
    try {
      const response = await fetch(`https://benneats-food-order.onrender.com/carts/${item._id}`, {
        method: "PUT",
        headers: {
    ...
    if (item.quantity > 1) {
      try {
        const response = await fetch(
          `https://benneats-food-order.onrender.com/carts/${item._id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDelete =   (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://benneats-food-order.onrender.com/carts/${item._id}`).then(response => {
          if (response) {
            refetch();
  ...
  return (
    <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4">
...

      {
        ...
        <Link to="/menu"><button className="btn bg-red text-white mt-3">Back to Menu</button></Link>
      </div>
      }
